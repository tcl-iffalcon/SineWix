...
catalogDefs.push({ id: "sw-search-series", type: "series", name: "SineWix — Dizi/Anime Ara" });
catalogDefs.push({ id: "sw-search-movie",  type: "movie",  name: "SineWix — Film Ara" });
...
function toMeta(item, forceKind) {
  ...
  return {
    ...
    releaseInfo: releaseInfo(item.release_date, item.release_date),
    genres:      genres(item.genre_name) || item.genreslist,
...
function getMeta(type, id) {
  var p = parseId(id);
  if (!p) return Promise.resolve({ meta: {} });
      ...
      .then(function(data) {
        return {
          meta: {
            ...
            genres:      genres(data.genresname) || data.genres,
    ...
    .then(function(data) {
      ...
      return {
        meta: {
          ...
          genres:      genres(data.genresname) || data.genreslist,
          videos:      videos
        }
      };
    })
...
function getStreams(type, id) {
  ...
  if (p.kind === "movie") {
    return apiGet("/media/detail/" + p.sid + "/" + API_KEY)
      .then(function(data) { return { streams: toStreams(data.videos) }; })
  ...
  // Dizi
  if (!p.season || !p.episode) return Promise.resolve({ streams: [] });
